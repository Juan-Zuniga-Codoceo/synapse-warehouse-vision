...
db.serialize(() => {
    ...
    db.run('DROP TABLE IF EXISTS inventory_log');
    db.run('DROP TABLE IF EXISTS locations');

    // Create locations table (removed sku field)
    db.run(`
  ...
  `, (err) => {
        ...
        else console.log('✓ Created locations table');
    });
  ...
  `, (err) => {
        ...
        else console.log('✓ Created inventory_items table');
    });

    // Create indexes for performance
    db.run('CREATE INDEX idx_sku ON inventory_items(sku)', (err) => {
        if (err) console.error('Error creating SKU index:', err);
        else console.log('✓ Created SKU index');
    });

    db.run('CREATE INDEX idx_expiration ON inventory_items(expiration_date)', (err) => {
        if (err) console.error('Error creating expiration index:', err);
        else console.log('✓ Created expiration index');
    });
  ...
  `, (err) => {
        ...
        else console.log('✓ Created inventory_log table');
    });
